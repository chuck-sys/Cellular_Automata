...
const char App_Info[] =
"Cellular Automata is a program which lets you experiment with\n"
...
Fl_Menu_Item Menu_Items[] = {
    ...
    {"Stamps", 0, 0, 0, FL_SUBMENU},
        ...
        {"Oscillators", 0, 0, 0, FL_SUBMENU},
            ...
            {"Pentadecathlon", 0, load_stamp_cb, (void*)"stamps/pendeca.ca"},
            {0},
        {"Spaceships", 0, 0, 0, FL_SUBMENU},
            {"Glider", 0, load_stamp_cb, (void*)"stamps/glider.ca"},
            {"LWSS", 0, load_stamp_cb, (void*)"stamps/lwss.ca"},
            {0},
        {"Others", 0, 0, 0, FL_SUBMENU},
            {"Gospel Glider Gun", 0, load_stamp_cb, (void*)"stamps/gggun.ca"},
            {0},
        {0},
        ...
        for(int x=tempfx; x<=tempsx; x++) {
            ...
            for(short ofy=-1; ofy<2; ofy++) {
                for(short ofx=-1; ofx<2; ofx++) {
                    if(((x+ofx>=0) && (y+ofy>=0) && (x+ofx <= tempsx) &&
                                (y+ofy <= tempsy) && !((ofx == 0) && (ofy == 0)))) {
